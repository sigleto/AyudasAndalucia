...
const InformePrestacionesDiscapacidad: React.FC = () => {
  const route = useRoute<RouteProp<Record<string, RouteParams>, string>>();
  const { discapacidad, residencia, renta, importeEstimado } =
    route.params || {};
  ...
  const generarPDF = async () => {
    try {
      const contenidoHTML = `
        ...
        <body>
          <h1>Informe de Prestaciones por Discapacidad</h1>
          ...
          <h3>Descripción:</h3>
          <p>Este simulador estima las prestaciones disponibles para personas con discapacidad que cumplen ciertos requisitos relacionados con el porcentaje de discapacidad, la residencia y la renta familiar.</p>
          <h3>Pasos para acceder a las prestaciones:</h3>
          <ol>
          ...
          <p><em>Nota:</em> Este informe es meramente informativo y no garantiza la concesión de la prestación.</p>
        </body>
        </html>
      `;
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Informe de Prestaciones por Discapacidad</Text>
...
export default InformePrestacionesDiscapacidad;
